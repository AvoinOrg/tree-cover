...
#    print(i)
    if (i-1)//10 != i//10:
        if i>15099: 
            ...
            burn = pd.read_csv(f"tree-cover/data/15103_2015-01-01_2015-12-31.csv",
                           sep=",", header=1)
        else: 
            print(f'Loading file no.{9+10*(i//10)}')
            burn = pd.read_csv(f"tree-cover/data/{9+10*(i//10)}_2015-01-01_2015-12-31.csv",
                           sep=",", header=1)
    dist = ((burn.longitude - df.longitude_x[i]
             #df.location_x[i]
             )** 2 + (burn.latitude - df.latitude_x[i]
            #df.location_y[i]
            )**2)
    idx_low = dist.idxmin()
    idx_up = idx_low + 12
