...
def single_point_populater(df, prep_method, aggregate_method,verbose=False):
    ...
    """
    if verbose: print('Populating data.')
...
def landsat_prepare_method(df, verbose=False):
    ...
    """
    if verbose: print('Preping data')
    info = {'collection': 'LANDSAT/LC08/C01/T1_SR',
            'cols': ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B10', 'B11', 'sr_aerosol',
                    'pixel_qa', 'radsat_qa'],
            }
    for _ in info['cols']:
...
def main():
    ...
    if len(df_ex) & len(df_ex) < 100:
        df_ex = single_point_populater(df_ex, landsat_prepare_method,
                                       landsat_aggregate_method,
                                       verbose=True)
    db.append(df_ex)
