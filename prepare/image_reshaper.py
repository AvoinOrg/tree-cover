...
# FOR RESHAPING AN ARRAY W/O ADDITIONAL TRANSFORMS
def reshape_image_array(img_arr, mode, params, size):
    return __reshape_image(img_arr, mode, [params, size])
...
def reshape_single_image(input_path,        # PATH OF THE INPUT IMAGE
    ...
    if save:
        imwrite(output_path, reshaped_image)
    if display:
        fig = plt.figure()
...
# %% PRIVATE FUNCTIONS
def __imresize(image, size, interp):
    resample = None
    if interp == 'nearest':
    ...
    return np.array(Image.fromarray(image).resize( size, resample=resample))
...
def __reshape_image(image,      # IMAGE ARRAY
    ...
    if (mode == 'interp'):
        reshaped_image = __imresize(image, params[1], interp=params[0][0])
    ...
    if (mode == 'tile'):
        ...
        reshaped_image = __imresize(__tile_image(image, start_pos), params[1])
    ...
    if (mode == 'mirror'):
        ...
        if params[0][0] == 'random':
            ...
            if (a[1]==a[0]):
                ...
                start_pos = np.random.randint(0, abs(a[1]-a[0]))
        reshaped_image = __imresize(__mirror_image(image, start_pos), params[1])
    ...
    if (mode == 'crop'):
        ...
        if params[0][0] == 'random':
            ...
            if (a[1]==a[0]):
                ...
                start_pos = np.random.randint(0, abs(a[1]-a[0]))
        reshaped_image = __imresize(__crop_image(image, start_pos), params[1])
    ...
    if (mode == 'contain'):
        ...
        if params[0][0] == 'random':
            ...
            if (a[1]==a[0]):
                ...
                start_pos = np.random.randint(0, abs(a[1]-a[0]))
        reshaped_image = __imresize(__contain_image(image, start_pos, params[0][1]), params[1])
    return reshaped_image
